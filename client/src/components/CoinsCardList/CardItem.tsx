...
import { Card, Text, BlockStack, Box, Badge, Thumbnail, Button } from "@shopify/polaris";
...
const CardItem: React.FC<CardItemProps> = ({ coinItem, onViewDetails }) => {
    ...
    <Card>
      <Box padding="400">
        <BlockStack>
          ...
          <BlockStack>
            <BlockStack>
              {coinItem.image && <Thumbnail source={coinItem.image} alt={coinItem.name} size="small" />}
              <BlockStack>
            ...
            <Badge icon={changeIcon}>{formatPercentage(coinItem.price_change_percentage_24h || 0)}</Badge>
          </BlockStack>
          ...
          {onViewDetails && (
            <Box>
              <Button fullWidth onClick={() => onViewDetails(coinItem)}>
                查看详情
              </Button>
            </Box>
          )}
